...
export function renderSession() {
  ...
  const isUnconscious = (c.statuses || []).some(
  ...
  document
    .getElementById("session-screen")
    .classList.toggle("edit-mode", editMode);
  document
    .getElementById("session-screen")
    .classList.toggle("character-locked", locked);
  document.getElementById("add-spellslot-btn").hidden = !editMode;
  document.getElementById("add-resource-btn").hidden = !editMode;
  document.getElementById("add-hitdice-btn").hidden = !editMode;
  document.getElementById("edit-character-btn").hidden = !editMode;
...
function longRest() {
  ...
  if (!c) return;
  c.resources.forEach((r) => {
    if (r.recoversOn !== "none") r.current = r.max;
  });
  c.spellSlots.forEach((s) => {
    s.used = 0;
  });
  ...
  if (c.hitDice && c.hitDice.length) {
    ...
    for (const size of HD_SIZES) {
      ...
      const r = Math.min(hd.spent, toRecover);
      hd.spent -= r;
      toRecover -= r;
    }
...
export function initSessionControls() {
  ...
  document.getElementById("long-rest").addEventListener("click", () => {
    const c = getSelectedCharacter();
    if (c && c.locked) return;
    if (!confirm("Take a long rest? This will restore HP and all resources."))
      return;
    longRest();
    showToast("Long rest taken — HP and resources restored");
    flashBar();
  });
}

...
export function coinsToCP(coins) {
  return COIN_ORDER.reduce(
    (sum, d) => sum + (coins[d] || 0) * COIN_IN_CP[d],
    0,
  );
}
...
function _spendCP(c, spendCP, preferDenom) {
  ...
  for (let i = prefIdx + 1; i < ORDER.length; i++) {
    ...
    remaining = 0;
    coins[preferDenom] =
      (coins[preferDenom] || 0) + Math.floor(change / prefVal);
  }
...
export function renderInventory(c) {
  ...
  COIN_ORDER.forEach((denom) => {
    const input = document.querySelector(
      `.coin-box[data-coin="${denom}"] .coin-input`,
    );
...
function _buildComponentCard(el, item, c, onDelete) {
  ...
  if (editMode) {
    ...
    editBtn.style.cssText =
      "border-color:var(--muted);color:var(--muted);width:100%;margin-top:0.4rem;";
    ...
    useBtn.addEventListener("click", () => {
      ...
      if (amount > item.gpAmount) {
        showToast(`Only ${item.gpAmount} gp left`);
        return;
      }
    ...
    useBtn.addEventListener("click", () => {
      if (c.locked) return;
      if (item.qty <= 0) {
        showToast(`No ${item.name} remaining`);
        return;
      }
      item.qty--;
      saveState();
    ...
    buyBtn.addEventListener("click", () => {
      ...
      showToast(
        cost > 0
          ? `Bought 1 ${item.name} for ${cost} gp`
          : `Added 1 ${item.name}`,
      );
    });
    actionRow.appendChild(useBtn);
...
function _openComponentEditModal(item, c) {
  ...
  document.getElementById("component-edit-modal-title").textContent =
    `Edit: ${item.name}`;
...
export function initInventoryControls() {
  ...
  document.getElementById("coin-add").addEventListener("click", () => {
    ...
    if (!c.coins) c.coins = { cp: 0, sp: 0, ep: 0, gp: 0, pp: 0 };
    const amount =
      parseInt(document.getElementById("coin-amount").value, 10) || 0;
  ...
  */
  ...
  (function () {
    const modal = document.getElementById("component-modal"),
      ...
      openBtn = document.getElementById("add-component-btn"),
      cancel = document.getElementById("component-cancel"),
      nameIn = document.getElementById("component-form-name"),
      gpIn = document.getElementById("component-form-gp"),
      qtyIn = document.getElementById("component-form-qty"),
      errEl = document.getElementById("component-form-error");
    const open = () => {
      form.reset();
      errEl.hidden = true;
      modal.hidden = false;
    ...
    form.addEventListener("submit", (e) => {
      ...
      if (isNaN(gp) || gp < 0) {
        errEl.textContent = "Please enter a valid GP amount.";
        errEl.hidden = false;
        return;
      }
      const c = getSelectedCharacter();
      ...
      const qtyVal = qtyIn.value.trim(),
        hasQty = qtyVal !== "" && parseInt(qtyVal, 10) >= 1;
      c.components.push(
        hasQty
          ? {
  ...
  (function () {
    const modal = document.getElementById("component-edit-modal"),
      form = document.getElementById("component-edit-form"),
    ...
    form.addEventListener("submit", (e) => {
      ...
      if (item.qty === null) {
        const gp = parseFloat(
          document.getElementById("component-edit-gp").value,
        );
        if (isNaN(gp) || gp < 0) {
          document.getElementById("component-edit-error").textContent =
            "Enter a valid GP amount.";
          document.getElementById("component-edit-error").hidden = false;
          return;
        }
        item.gpAmount = gp;
      } else {
        item.price = Math.max(
          0,
          parseFloat(document.getElementById("component-edit-price").value) ||
            0,
        );
        item.qty = Math.max(
          0,
          parseInt(document.getElementById("component-edit-qty").value, 10) ||
            0,
        );
      }
      saveState();

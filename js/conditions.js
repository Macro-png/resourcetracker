...
export function renderStatuses(c) {
  ...
  STANDARD_CONDITIONS.forEach((cond) => {
    ...
    if (implied) {
      btn.classList.add("implied");
      btn.title = "Applied by another condition";
    }
  ...
  if (banner) {
    banner.hidden = !conc;
    if (conc)
      banner.textContent = `${c.name} is concentrating — taking damage requires a CON save to maintain it.`;
  }

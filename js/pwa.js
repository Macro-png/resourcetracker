...
function parseAuroraFile(xmlText) {
  ...
  if (parseError)
    throw new Error(
      "Could not parse XML: " + parseError.textContent.slice(0, 80),
    );
  ...
  const displayProps =
    xml.getElementsByTagName("display-properties")[0] || null;
  const buildInput = xml.getElementsByTagName("input")[0] || null;

  const name =
    firstText(displayProps, "name") ||
    firstText(buildInput, "name") ||
    "Unknown";
  ...
  const levelEls = Array.from(xml.getElementsByTagName("element")).filter(
    (el) => el.getAttribute("type") === "Level" && el.getAttribute("rndhp"),
  );

  levelEls.sort(
    (a, b) =>
      parseInt(a.getAttribute("name"), 10) -
      parseInt(b.getAttribute("name"), 10),
  );
  ...
  while ((m = CLASS_RE.exec(classLine)) !== null) {
    ...
    else if (HALF_CASTERS.some((c) => cls.includes(c)))
      casterLevels.half += lvl;
    else if (PACT_CASTERS.some((c) => cls.includes(c)))
      casterLevels.pact += lvl;
  }
    ...
    .addEventListener("change", async (ev) => {
      ...
      try {
        ...
        if (file.name.endsWith(".dnd5e")) {
          ...
          const idx = existing.characters.findIndex(
            (c) => c.name === char.name,
          );
